...
export class TokenService {
  ...
  // 토큰 발급
  async generateTokens(userId: string, userType: string) {
    const accessTokenPayload = this.createTokenPayload(userId, userType);
    const refreshTokenPayload = this.createTokenPayload(userId, userType);
  ...
  // 페이로드 생성
  private createTokenPayload(userId: string, userType: string) {
    return {
      sub: userId,
      userType,
