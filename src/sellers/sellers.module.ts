...
const CommandHandlers = [RegisterSellerHandler, UpdateSellerProfileHandler, VerifyBusinessNumberHandler];
const QueryHandlers = [];
const EventHandlers = [SellerRegisteredHandler, SellerProfileUpdatedHandler, VerifyBusinessNumberCommand];

@Module({
  imports: [
    CqrsModule,
    TypeOrmModule.forFeature([Seller]),
    MongooseModule.forFeature([{ name: SellerView.name, schema: SellerViewSchema }]),
    EventStoreModule,
    RedisModule
  ],
