...
export class HmacUtil {
  ...
  verify(email: string, verificationCode: string, expirationTime: Date, signature: string): boolean {
    const expectedSignature = this.sign(email, verificationCode, expirationTime);
