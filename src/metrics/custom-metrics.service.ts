...
export class CustomMetricsService {
  ...
  incrementHttpRequests(method: string, path: string, statusCode: number) {
    this.httpRequestsCounter.inc({ method, path, status_code: statusCode.toString() });
    this.logger.debug(`Incremented http_requests_total: ${method} ${path} ${statusCode}`);
  }

  observeHttpRequestDuration(method: string, path: string, statusCode: number, duration: number) {
    this.httpRequestDurationHistogram.observe({ method, path, status_code: statusCode.toString() }, duration);
    this.logger.debug(`Observed http_request_duration_seconds: ${method} ${path} ${statusCode} ${duration}`);
  }
