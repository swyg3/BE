...
@Injectable()
export class DyUserViewRepository {
  private readonly logger = new Logger(DyUserViewRepository.name);

  constructor(
    @InjectModel('UserView')
    private readonly userViewModel: Model<DyUserView, { userId: string }>
  ) {}

  async create(userView: DyUserView): Promise<DyUserView> {
    try {
  ...
  async findByUserId(userId: string): Promise<DyUserView | null> {
    try {
  ...
  async findByEmail(email: string): Promise<DyUserView | null> {
    try {
  ...
  async update(userId: string, updates: Partial<DyUserView>): Promise<DyUserView | null> {
    try {
  ...
  async findOneAndUpdate(
    userId: string,
    userView: Partial<DyUserView>
  ): Promise<{ userView: DyUserView; isNewUserView: boolean }> {
